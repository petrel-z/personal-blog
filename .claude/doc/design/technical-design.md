...
model Role {
  id          String   @id @default(cuid())
  name        String   @unique // admin, editor, reviewer
  description String?
  userRoles   UserRole[]
...
model Permission {
  id          String   @id @default(cuid())
  code        String   @unique // post:create, post:update, comment:approve
  description String?
  rolePermissions RolePermission[]
...
model Post {
  ...
  deletedAt   DateTime?  // 软删除时间（30 天后物理删除）
  ...
  // 索引优化
  @@index([slug])
  @@index([status, publishedAt])      // 首页查询：已发布 + 时间排序
  @@index([categoryId])               // 分类页查询
  @@index([authorId])                 // 作者文章列表
  @@index([viewCount])                // 热度排行
  @@index([isPinned, publishedAt])    // 置顶文章优先
  @@index([deletedAt])                // 回收站查询
}
...
model Comment {
  ...
  // 索引优化
  @@index([postId, status])         // 文章评论列表（按状态筛选）
  @@index([postId, createdAt])      // 文章评论时间线
  @@index([status, createdAt])      // 后台审核列表
  @@index([parentId])               // 子评论查询
}
...
model AuditLog {
  id        String   @id @default(cuid())
  userId    String?
  action    String   // post.create, post.delete, comment.approve
